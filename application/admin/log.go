...
func GetLogList(c *gin.Context) {
	...
	sql := "select id, sid, content, phone, time, task_id, receive, receive_time, decrease_num, `status`,uid from sent_log where time between ? and ?"
	count := "select count(*) from sent_log where time between ? and ? "
	if uid != "" {
		sql += " and uid = ?"
		count += " and uid = ?"
	...
	for rows.Next() {
		err := rows.Scan(&sentLog.Id, &sentLog.Sid, &sentLog.Content, &sentLog.Phone, &sentLog.Time, &sentLog.TaskId, &sentLog.Receive, &sentLog.ReceiveTime, &sentLog.DecreaseNum, &sentLog.Status, &sentLog.Uid)
		if err != nil {
			common.Log(c, err.Error())
			c.JSON(200, gin.H{
				"status": -1,
